...
describe("PMM Test", function() {
    ...
    let tokenApprove, dexRouter, wNativeRelayer, tokenApproveProxy, smartRouter, uniAdapter;
    ...
    it("1.3 ERC20 Exchange By FixRate, Wrong Price Source Passed", async () => {
        await initMockTokens();
        await initUniSwap();
        await addLiquidity();
        // alice: taker, bob: maker
    ...
    it("1.4 ERC20 -> Native Exchange By FixRate", async () => {
        await initMockTokens();
        await initUniSwap();
        await addLiquidity();
        // alice: taker, bob: maker
    ...
    it("1.5 Native -> ERC20 Exchange By FixRate", async () => {
        await initMockTokens();
        await initUniSwap();
        await addLiquidity();
        // alice: taker, bob: maker
    ...
    it("1.6 Cancel Quotes And Query Order Status", async () => {
        await initMockTokens();
        await initUniSwap();
        await addLiquidity();
        // alice: taker, bob: maker
    ...
    it("1.7 Fill the same order more than twice", async () => {
        await initMockTokens();
        await initUniSwap();
        await addLiquidity();
        // alice: taker, bob: maker
    ...
    it("2.1 ERC20 Exchange By FixRate by smart swap", async () => {
        ...
        let takerToAmount0 = await toToken.balanceOf(alice.address);
        let makerToAmount0 = await toToken.balanceOf(bob.address);
        let tx = await dexRouter.connect(alice).smartSwap(
            0,
            baseRequest,
            [swapAmount],
            [[]],
    ...
    it("2.10 ERC20 Exchange By Invest whole replaced by pmm", async () => {
        ...
        await fromToken.connect(alice).transfer(smartRouter.address, swapAmount);
        let tx = await smartRouter.connect(alice).smartSwapByInvest(
            baseRequest,
            [swapAmount],
            [[]],
            [request],
    ...
    it("2.11 In case of quote expired", async () => {
        ...
        let request = [
            ...
            infos.fromTokenAmountMax, 
            infos.toTokenAmountMax, 
            100,    // an expired deadline, forcing it fall back to dex
            infos.extension
        ];
        const pmmRequests = [request];
    ...
    const initWNativeRelayer = async () => {
        ...
        await wNativeRelayer.connect(wNativeRelayerOwner).setCallerOk([dexRouter.address, smartRouter.address], [true, true]);
