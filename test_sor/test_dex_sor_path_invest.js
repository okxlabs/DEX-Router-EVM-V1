...
describe("Smart route path test", function() {
  ...
  it("mixSwap with single path with safemoon token", async () => {
    ...
    await fromToken.connect(alice).transfer(dexRouter.address, fromTokenAmount);
    ...
    await dexRouter.connect(alice).smartSwapByInvest(
        baseRequest,
        [fromTokenAmount],
        [layer1],
        [],
  ...
  it("mixSwap with two fork path", async () => {
    ...
    rxResult = await dexRouter.connect(alice).smartSwapByInvest(
        baseRequest,
  ...
  it("mixSwap with four path, same token", async () => {
    ...
    await wbtc.connect(alice).transfer(dexRouter.address, fromTokenAmount);
    // node1
    const mixAdapter1 = [
      uniAdapter.address,
