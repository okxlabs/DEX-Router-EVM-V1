...
describe("Smart route path test with commission", function () {
  ...
  it("mixSwap with single path with safemoon token with commission", async () => {
    ...
    expect(await fromToken.balanceOf(feeReceiver)).to.be.equal(commissionAmount.mul(989).div(1000))
    expect(await toToken.balanceOf(tom.address)).to.be.equal("986056732423396135312")
  ...
  it("mixSwap with single path with commission", async () => {
    ...
    expect(await usdt.balanceOf(tom.address)).to.be.eq(receive1.toString());
  });
  ...
  it("mixSwap with two fork path with commission", async () => {
    ...
    const fromTokenAmount1 = fromTokenAmount.div(2);
    const fromTokenAmount2 = fromTokenAmount.div(2);
    const minReturnAmount = ethers.utils.parseEther('0');
    const deadLine = FOREVER;
    ...
    expect(await usdt.balanceOf(tom.address)).to.be.eq(receive);
  });
  ...
  it("mixSwap with three fork path with commission", async () => {
    ...
    expect(await toToken.balanceOf(dexRouter.address)).to.be.eq("0");
    expect(await toToken.balanceOf(tom.address)).to.be.eq("53597548250295474132461");
    expect(await fromToken.balanceOf(feeReceiver)).to.be.eq(commissionAmount)
  });
  ...
  it("mixSwap with single path and source token is native token with commission", async () => {
    ...
    expect(await toToken.balanceOf(tom.address)).to.be.eq(receive0);
    expect(await ethers.provider.getBalance(feeReceiver)).to.be.eq(commissionAmount)
  });
  ...
  it("mixSwap with single path and target token is native token with commission", async () => {
    ...
    expect(await ethers.provider.getBalance(tom.address)).to.be.gte(finalAmount);
    expect(await fromToken.balanceOf(feeReceiver)).to.be.eq(commissionAmount)
  });
  ...
  it("mixSwap with single path with order id with commission", async () => {
    ...
    expect(await toToken.balanceOf(dexRouter.address)).to.be.eq("0");
    expect(await toToken.balanceOf(tom.address)).to.be.eq("986056732423396135312")
    expect(await fromToken.balanceOf(feeReceiver)).to.be.eq(commissionAmount.mul(989).div(1000))
  });
