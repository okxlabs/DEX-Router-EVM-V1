...
contract BiAdapter is IAdapter {
  ...
  function sellBase(
    ...
    (uint256 reserveIn, uint256 reserveOut, ) = IBiswapPair(pool).getReserves();
    ...
    uint256 sellBaseAmountWithFee = sellBaseAmount * (1000 - IBiswapPair(pool).swapFee());
  ...
  function sellQuote(
    ...
    (uint256 reserveOut, uint256 reserveIn, ) = IBiswapPair(pool).getReserves();
    require(reserveIn > 0 && reserveOut > 0, "BiswapAdapter: INSUFFICIENT_LIQUIDITY");

    uint256 balance1 = IERC20(quoteToken).balanceOf(pool);
    uint256 sellQuoteAmount = balance1 - reserveIn;

    uint256 sellQuoteAmountWithFee = sellQuoteAmount * (1000 - IBiswapPair(pool).swapFee());
