...
contract KyberAdapter is IAdapter {
  ...
  function sellBase(
    ...
    (uint256 reserveIn, uint256 reserveOut, uint256 vReserveIn, uint256 vReserveOut, uint256 feeInPrecision) = IKyber(
      pool
    ).getTradeInfo();
    ...
    uint256 sellBaseAmountWithFee = (balance0 - reserveIn) * (PRECISION - feeInPrecision);
  ...
  function sellQuote(
    ...
    (uint256 reserveOut, uint256 reserveIn, uint256 vReserveOut, uint256 vReserveIn, uint256 feeInPrecision) = IKyber(
      pool
    ).getTradeInfo();
    require(reserveIn > 0 && reserveOut > 0, "KyberAdapter: INSUFFICIENT_LIQUIDITY");
    ...
    uint256 sellQuoteAmountWithFee = (balance1 - reserveIn) * (PRECISION - feeInPrecision);
