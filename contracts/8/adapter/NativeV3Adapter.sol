...
contract NativeV3Adapter is IAdapter, INativeV3SwapCallback {
    ...
    function _nativeV3Swap(
        ...
        (address fromUnderlyingToken, address toUnderlyingToken, uint24 fee) = abi.decode(
            data,
            (address, address, uint24)
        );

        // get the lpToken of the underlying tokens
        address fromToken = INativeV3CreditVault(CREDIT_VAULT).lpTokens(fromUnderlyingToken);
        address toToken = INativeV3CreditVault(CREDIT_VAULT).lpTokens(toUnderlyingToken);

        // get the amount of LP token for a given underlying token balance
        uint256 sellAmount = INativeLPToken(fromToken).getSharesByUnderlying(IERC20(fromUnderlyingToken).balanceOf(address(this)));
        ...
        INativeLPToken(toToken).redeemTo(INativeLPToken(toToken).sharesOf(address(this)), to);

        // redeem the remaining LP tokens to the payerOrigin
        uint remainingFromShares = INativeLPToken(fromToken).sharesOf(address(this));
        if (remainingFromShares > 0 && _payerOrigin != address(0)) {
            INativeLPToken(fromToken).redeemTo(remainingFromShares, _payerOrigin);
        }
    }
    ...
    function pay(
        ...
        uint256 underlyingAmount = INativeLPToken(token).getUnderlyingByShares(value);
        ...
        if (payer == address(this)) {
            ...
            SafeERC20.safeTransferFrom(IERC20(underlying), payer, address(this), underlyingAmount);
            INativeLPToken(token).depositFor(recipient, underlyingAmount);
        }
    }
}
