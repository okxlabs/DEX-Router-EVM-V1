...
contract PMMAdapter is IAdapterWithResult {
  ...
  function _pmmSwap(
    ...
    (IMarketMaker.PMMSwapRequest memory request, bytes memory signature) = abi.decode(
      moreInfo,
      (IMarketMaker.PMMSwapRequest, bytes)
    );
    ...
    uint256 result = IMarketMaker(marketMaker).swap(sellAmount, request, signature);
