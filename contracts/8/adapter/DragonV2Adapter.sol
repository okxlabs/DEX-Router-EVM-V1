...
import "../interfaces/IDragonV2SwapCallback.sol";
...
contract DragonV2Adapter is IAdapter, IDragonV2SwapCallback {
    address internal constant Sei_ADDRESS = 0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE;
    address public immutable WSEI;

    constructor(address payable wsei) {
        WSEI = wsei;
    }

    function _dragonV2Swap(
        address to,
        address pool,
        uint160 sqrtX96,
        bytes memory data
    ...
    function sellBase(
        ...
        _dragonV2Swap(to, pool, sqrtX96, data);
    }
    ...
    function pay(
        ...
        if (token == WSEI && address(this).balance >= value) {
            // pay with WETH9
            IWETH(WSEI).deposit{value: value}(); // wrap only what is needed to pay
            IWETH(WSEI).transfer(recipient, value);
