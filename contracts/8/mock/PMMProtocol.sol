    ...
    EIP712Upgradeable
{
    ...
    // initialize
    function initialize(IWETH weth) public initializer {
        __Pausable_init();
    ...
    receive() external payable {
        if (msg.sender != address(_WETH)) revert Errors.EthDepositRejected();
    }
        ...
        )
    {
        ...
        if (flagsAndAmount & _SIGNER_SMART_CONTRACT_HINT != 0) {
            ...
            if (!ECDSA.recoverOrIsValidSignature(order.maker, orderHash, r, vs))
                revert RFQBadSignature();
        }
