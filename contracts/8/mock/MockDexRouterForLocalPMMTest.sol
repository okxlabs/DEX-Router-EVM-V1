    ...
    ReentrancyGuardUpgradeable
{
    ...
    function initialize(address _temp_weth, address _w_native_relayer)
        public
        initializer
    {
        __Ownable_init();
    ...
    function _exeHop(
        ...
        for (uint256 i = 0; i < hops.length; i++) {
            ...
            if (isHopReplace(hops[i].fromToken)) {
                ...
                if (
                    _tryPmmSwap(fromToken, batchAmount, extraData[pmmIndex]) ==
                    0
                ) {
                    continue;
                }
            ...
            if (amount > 0) {
                ...
                if (to != address(this)) {
                    SafeERC20.safeTransfer(
                        IERC20(address(uint160(temp_weth))),
                        to,
                        amount
                    );
                }
    ...
    function _transferTokenToUser(address token) internal {
        if ((IERC20(token).isETH())) {
            uint256 wethBal = IERC20(address(uint160(temp_weth))).balanceOf(
                address(this)
            );
            if (wethBal > 0) {
                IWETH(address(uint160(temp_weth))).transfer(
                    address(uint160(w_native_relayer)),
                    wethBal
                );
                IWNativeRelayer(address(uint160(w_native_relayer))).withdraw(
                    wethBal
                );
            }
    ...
    function _tryPmmSwap(
        ...
        errorCode = IAdapterWithResult(pmmAdapter).sellBase(
            address(this),
            address(0),
            moreInfo
        );
        ...
        returns (uint256 returnAmount)
    {
        ...
        if (isReplace(localBaseRequest.fromToken)) {
            ...
            if (
                ...
                returnAmount =
                    IERC20(baseRequest.toToken).universalBalanceOf(msg.sender) -
                    returnAmount;
                require(
