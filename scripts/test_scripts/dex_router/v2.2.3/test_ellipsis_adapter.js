...
async function MetaPool() {
    ...
    console.log("before Account USDT Balance: " + await USDT.balanceOf(account.address));
    console.log("before Account TUSD Balance: " + await TUSD.balanceOf(account.address));
    ...
    let minReturnAmount = 0;
    let deadLine = FOREVER;
    let poolAddress = "0x3F52c27C0634C897Faf776b11bE1Dd408E7a429e"; 
    ...
    let rawData1 = [
        "0x" + 
        direction(tokenConfig.tokens.USDT.baseTokenAddress, tokenConfig.tokens.TUSD.baseTokenAddress) + 
        "0000000000000000000" + 
        weight1 + 
        poolAddress.replace("0x", "")  // three pools
    ];
    ...
    console.log("after Account USDT Balance: " + await USDT.balanceOf(account.address));
    console.log("after Account TUSD Balance: " + await TUSD.balanceOf(account.address));
}
