...
contract CamelotV3UniversalAdapterTest is UniversalAdapterTestUtils {
    ...
    function test_camelotV3_swapWETHtoUSDC_customAdapter() public user(bob) {
        ...
        SwapInfo memory swapInfo = genSwapInfo(address(customAdapter), WETH, USDC, WETH_USDC, amount);
        ...
        console2.log("toToken balance after", IERC20(USDC).balanceOf(address(bob)));
    }
    ...
    function test_camelotV3_swapWETHtoUSDC_universalAdapter() public user(bob) {
        ...
        uint256 amount = IERC20(WETH).balanceOf(bob);
        SwapInfo memory swapInfo = genSwapInfo(address(universalAdapter), WETH, USDC, WETH_USDC, amount);
