...
contract Deploy is Test {
    ...
    function deploy_bsc() internal {
        ...
        vm.broadcast(deployer);
        address adapter = address(new NativePmmAdapter(WBNB));
        console2.log("NativePmmAdapter deployed ethereum: %s", adapter);
    }
    function deploy_base() internal {
        ...
        vm.broadcast(deployer);
        address adapter = address(new NativePmmAdapter(WETH));
        console2.log("NativePmmAdapter deployed base: %s", adapter);
    }
    function deploy_arb() internal {
        ...
        vm.broadcast(deployer);
        address adapter = address(new NativePmmAdapter(WETH));
        console2.log("NativePmmAdapter deployed arbi: %s", adapter);
    }
}
